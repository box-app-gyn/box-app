...
function sanitizeAndValidateString(value, fieldName, maxLength = 1000) {
    ...
    if (security_1.SECURITY_UTILS.containsSuspiciousPattern(trimmed)) {
        return { isValid: false, error: `${fieldName} contém conteúdo suspeito` };
    }
    // Sanitizar string
    const sanitized = security_1.SECURITY_UTILS.sanitizeString(trimmed, maxLength);
